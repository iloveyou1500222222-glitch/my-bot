...
# --- 🛡 အရေးပေါ် Admin Tools (ချစ်စရာကောင်းတဲ့ စာသားများ) ---
    ...
    try:
        if '/ban' in cmd:
            ...
            await update.message.reply_text(f"🚨 **သဲလေးရဲ့ အမိန့်တော်ပါရှင်** 🚨\n\n"
                                          f"@{target.username} ရေ... နင်ကလည်း ဒီ Group လေးနဲ့ မတန်ပါဘူးကွာ... "
                                          f"ငါ့မျက်စိရှေ့ကနေ အပြီးအပိုင် ထွက်သွားပေးတော့နော်! 🤬💢🔥")
        elif '/mute' in cmd:
            ...
            await update.message.reply_text(f"🤫 **ချစ်စရာလေးရဲ့ အမိန့်** 🤫\n\n"
                                          f"@{target.username} ရေ... စကားတွေ အရမ်းများနေလို့ သဲလေး စိတ်ညစ်ရတယ်... "
                                          f"ခဏလေးတော့ အသံတိတ်ပြီး လိမ်လိမ်မာမာလေး နေပေးပါဦးနော်! 🤐🔇🚫")
        elif '/umute' in cmd:
            ...
            await update.message.reply_text(f"🔊 **ပြန်ချစ်ပေးလိုက်ပြီနော်** 🔊\n\n"
                                          f"@{target.username} ရေ... အခုတော့ လိမ်မာသွားပြီထင်လို့ "
                                          f"ပြန်ပြီး စကားလေး ပြောခွင့်ပေးလိုက်ပါပြီနော်... အာဘွား! 😘✅✨")
    except: pass

# --- 📢 အချစ်ပို Admin ခေါ်ရန် စနစ် ---
async def call_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(f"📣📣 **Admin လေးတို့ရေ... သဲလေးတို့ အကူအညီလိုနေလို့ပါရှင်** 🥰\n\n"
                                      f"အသည်းလေးတို့ရယ်... မြန်မြန်လေး လာခဲ့ပေးပါဦးနော်... "
                                      f"သဲလေး စောင့်နေလို့ပါရှင့်... အာဘွား! 💋✨\n\n{mentions}")
    except: pass
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    start_msg = (
        f"မဂ်လာပါရှင့် 😘\n\n"
        f"🎀**မြနှင်း**🎀 ကို Group ထဲထည့်ပေးပါနော်။ အာဘွား😘 သဲယေး👉🥺👈။\n"
        f"အသုံးပြုပုံကြည့်ရန် - /help နှိပ်ပေးပါရှင့်။"
        f"{AD_TEXT}"
    )
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    help_msg = (
        "✨ **မြနှင်း Bot အသုံးပြုနည်းလမ်းညွှန်** ✨\n\n"
