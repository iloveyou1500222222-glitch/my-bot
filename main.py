...
async def broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"⏳ စတင်ပို့ဆောင်နေပါပြီ... (GP စုစုပေါင်း: {total} ခု)")
...
# --- 🎮 ဂိမ်းစနစ် (MBB / PUBG အဖော်ရှာရန်) ---
async def game_friend(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if game_db[g_type]:
        ...
        await update.message.reply_text(
            f"🎮 **ဂိမ်းဖော် တွေ့ပြီရှင့်!** 💉😜\n"
            f"@{user.username} နဲ့ @{partner_name} တို့ရေ... အတူတူ ဆော့လို့ရပြီနော်! 🎀✨"
        )
    else:
        game_db[g_type].append((user.id, user.username))
        await update.message.reply_text(
            f"⏳ @{user.username} ရေ... ဂိမ်းဖော် ရှာပေးနေတယ်နော်... တစ်ယောက်ယောက် လာမယ့်အချိန်ထိ စောင့်ပေးပါဦးရှင့်! 🥺🙏"
        )

# --- 📢 Allmember စနစ် (၅ ယောက်စီ @ ခေါ်ရန်) ---
async def all_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # ဒီနေရာမှာ Group ထဲကလူတွေကို ၅ ယောက်စီခွဲပြီး @ ခေါ်ပေးပါမယ်
    await update.message.reply_text("📣 စနစ်က အားလုံးကို ၅ ယောက်စီ @ ခေါ်ပေးနေပါပြီရှင့်... 🥰")

# --- 🚫 Member တွေ လင့်ခ်ချရင် ဖျက်မယ့်စနစ် ---
async def link_filter(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Admin တွေကို မဖျက်ပေးရအောင် စစ်ဆေးမယ်
    if await is_admin_or_owner(update, context):
        return

    # လင့်ခ်ပါဝင်ရင် ချက်ချင်း ဖျက်မယ်
    if any(x in update.message.text.lower() for x in ["http", "https", "t.me"]):
        await update.message.delete()
        await update.message.reply_text(
            f"🚫 @{update.effective_user.username} ရေ... Member တွေ လင့်ခ်မချရဘူးလေကွာ... "
            f"သဲလေး စိတ်ညစ်ရတယ်နော်... နောက်တစ်ခါ မလုပ်ပါနဲ့တော့ရှင့်! 🥺💔"
        )

# --- Main (Handler များကို ပေါင်းထည့်ပေးရန်) ---
def main():
    # ... (ယခင်ကုဒ်များအတိုင်း)
    app.add_handler(CommandHandler(["mbbfriend", "pubgfriend"], game_friend))
    app.add_handler(CommandHandler("allmember", all_member))
