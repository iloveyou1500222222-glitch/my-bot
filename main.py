...
WELCOME_VIDEO = "https://t.me/tear80808/264" 

# ကြော်ငြာစာသား (start တစ်ခုတည်းမှာပဲ သုံးပါမယ်)
...
# --- ✨ လူသစ်ဝင်လာလျှင် Video ဖြင့် ကြိုဆိုခြင်း ---
async def welcome_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    for member in update.message.new_chat_members:
        ...
        welcome_msg = (
            ...
            f"ပျော်ပျော်ရွှင်ရွှင်နဲ့ ဖူးစာရှာနိုင်ပါပြီနော်... 🥰"
        )
        ...
        try:
            await context.bot.send_video(
                ...
                video=WELCOME_VIDEO,
                caption=welcome_msg, # လူသစ်ဝင်လာချိန် ကြော်ငြာမပြပါ
                parse_mode='Markdown'
            )
        except:
...
async def broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ ပို့စ်ဖြန့်ဝေပြီးပါပြီရှင့်။")

# --- Group ID မှတ်သားခြင်း ---
...
async def love_match(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not potential:
        await update.message.reply_text("⏳ ဖူးစာရှင်လေးတွေ ရှာမတွေ့သေးပါဘူး သဲလေး... 🥺💗")
        return
    partner_id = random.choice(potential)
    # ကြော်ငြာစာသား ဖျောက်ထားသည်
    await update.message.reply_text(f"🎀 မြှားနတ်မောင်မြှားပစ်လိုက်ပါပီရှင့် 🎀\n\nအတွဲလေးကတော့ {me['username']} နဲ့ {registered_users[chat_id][partner_id]['username']} တို့ပဲ ဖြစ်ပါတယ်ရှင့်! 🤪")

# --- Start (ဒီမှာပဲ ကြော်ငြာပြပါမယ်) ---
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.type in ["group", "supergroup"]: 
        group_list.add(update.effective_chat.id)
    # ✅ Start မှာသာ ကြော်ငြာစာသား ပြမည်
